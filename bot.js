...
// Функция для обработки изображения (для photo и document)
const processImage = async (ctx, fileId) => {
    ...
    if (state && state.step === 'waitingForPhoto') {
        try {
            ...
            logger.info('Изображение успешно загружено', { userId, fileId });
        } catch (error) {
            logger.error('Ошибка загрузки изображения:', { error: error.message, userId });
            ctx.reply('Ошибка при загрузке изображения. Попробуй снова.');
        }
    } else {
...
bot.on('text', async (ctx) => {
    ...
    if (state.step === 'waitingForName') {
        ...
        ctx.reply(`Привет, ${state.client}! Отправь фото с информацией LMS (как фото или файл). https://lms.tuit.uz/student/info`);
...
bot.on('document', async (ctx) => {
    ...
    if (state && state.step === 'waitingForPhoto') {
        ...
        const isImage = validMimeTypes.includes(document.mime_type) ||
                        validExtensions.some(ext => fileName.toLowerCase().endsWith(ext));

        if (!isImage) {
            ctx.reply('Пожалуйста, отправь файл в формате изображения (JPEG или PNG).');
            return;
        }
